...
int main(int argc, char** argv)
{
  ...
  {
    Eigen::Affine3d pose;
    pose = Eigen::Translation3d(0.3, 0.3, .2 + 0.05 * i);
  ...
  {
    Eigen::Affine3d pose;
    pose = Eigen::Translation3d(0.3, 0.3 + 0.04 * i, 0.64);
    descartes_core::TrajectoryPtPtr pt = makeTolerancedCartesianPoint(pose);
    points.push_back(pt);
  }
  */
  ...
  Eigen::Affine3d pose;
  pose = Eigen::Translation3d(0.3, 0.3, .2);
  std::cout << "arm degree of freedom is: " << model->getDOF() << std::endl;
  std::vector<std::vector<double>> joint_poses;
  //model->getAllIK(pose,joint_poses);
  std::cout << "solution size is: " << joint_poses.size() << std::endl;
